...
while true; do
  ...
  case $yn in
  [Yy]* ) break;;
  [Nn]* )  echo && echo -e "\e[1;91mThanks for using Tool." ;exit;;
...
if [ "$options" = "$one" ]
  ...
  then
    ...
    echo -e
    echo -e
           nmap $firstip | cut -f6 -d":" | cut -f1 -d"N"
...
if [ "$options" = "$two" ]
  ...
  then
      ...
      echo -e
      echo -e
             nmap -T4 -F $firstip | cut -f6 -d":" | cut -f1 -d"N"
...
if [ "$options" = "$three" ]
  ...
  then 
     ...
     echo -e
     echo -e
            nmap -sV -T4 -O -F --version-light 10 $firstip | cut -f6 -d":" | cut -f1 -d"N"
...
if [ "$options" = "$four" ]
  ...
  then 
      ...
      echo -e
      echo -e
             nmap -sn --traceroute 10 $firstip | cut -f6 -d":" | cut -f1 -d"N"
